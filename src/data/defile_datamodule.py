...
class ForecastDataset(Dataset):
    def __init__(
        ...
        self.era5_daily_trans = self.transform_data["daily"].apply_transformers(self.era5_daily)
...
class DefileDataModule(LightningDataModule):
    ...
    def read_counts(self):
        all_count = pd.read_csv(
            self.data_dir + "/count/all_count_processed.csv",
            parse_dates=["date", "start", "end"],
        )

        # Filter data by years and day of year
        all_count = all_count[
            all_count["date"].dt.year.isin(self.years)
            & all_count["date"].dt.day_of_year.between(*self.doy)
        ]

        # Filter data by species and sum count of all species happening during the same period
        count_sp = (
            all_count[all_count.species == self.species][["date", "count", "start", "end"]]
            .groupby(["date", "start", "end"], as_index=False)["count"]
            .sum()
        )
        if len(count_sp) == 0:
            raise ValueError(f"No data for species {self.species} in the selected years.")
